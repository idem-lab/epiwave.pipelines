calculate_days_infection <- function(delay_list) {
    ...
    earliest_date <- as.Date(do.call(
        min, lapply(delay_list, function(df) min(rownames(df)))))
    ...
    latest_date <- as.Date(do.call(
        max, lapply(delay_list, function(df) max(rownames(df)))))
